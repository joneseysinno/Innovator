...
use hypernode::{EdgeId, EdgeKind, HyperEdge, HyperNode, NodeId, PropValue, SpaceClass};
...
pub fn handle_analysis_action(shell: &mut AppShell, action: AnalysisAction) {
    ...
    match action {
        ...
        AnalysisAction::ConfirmFieldBuilder => {
            let wall_id = shell.workspaces[idx].structural().and_then(|ws| ws.active_wall);
            let draft = shell.workspaces[idx]
                .structural_mut()
                .and_then(|ws| ws.field_builder.take());
            if let (Some(wall_id), Some(draft)) = (wall_id, draft) {
                ...
                if let Some(node) = shell.graph.nodes.get_mut(&wall_id) {
...
pub fn handle_promote_prop(shell: &mut AppShell, key: String) {
    ...
    let wall_id = match shell.workspaces[idx].structural().and_then(|ws| ws.active_wall) {
        Some(id) => id,
        None => return,
    };
    ...
    let ids: Vec<NodeId> = shell
        ...
        .collect();
    for id in &ids {
        if let Some(node) = shell.graph.nodes.get_mut(id) {
    ...
    shell.graph.insert_edge(HyperEdge {
        id: EdgeId(0),
        kind: EdgeKind::Wave,
        sources: vec![wall_id],
        targets: ids,
    ...
    let status = format!("signal: Wave promote {key}");
